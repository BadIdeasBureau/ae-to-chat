import { i18n } from '../ae-to-chat';
import { MODULE_NAME } from './settings';

//output effects to chat at the start of a token's turn (with timeout to guard against hilarity when deleting a token)

export const updateCombatHanfler = function(tracker){

	if((game.settings.get(MODULE_NAME, "startTurn") === "none") 
	...
	if (
		...
		(game.settings.get(MODULE_NAME,"startTurn")=== "linked" && !tokenData.actorLink) || //setting is set to linked only and actor is not linked
		(game.settings.get(MODULE_NAME,"startTurn")=== "player" && !(new Token(tokenData).actor.hasPlayerOwner)) 
	){ 
...
export let effectArray = [];  // Expected array structure: [{actor: a, effect: effectData}], which will be deduped later to prevent race conditions
...
export const processEffectArray = function(){
	...
	//console.log(newArray);
	if (game.settings.get(MODULE_NAME, "onApply")==="player") newArray.filter(e=>e.actor.hasPlayerOwner); //filter to players only if that option is set
	newArray.forEach(o => {
...
export const getTokenDataSceneFromActor = function(actor){
	...
	const scene = new Scene(linkedToken.scene,null);
...
export const printActive = async function(effects, tokenData, scene, hookType) {
	let subtitle;
	switch (hookType){ //allows adding a subtitle depending on the hook used (set as a variable, not autodetected).  Current could be an if statement, but, yknow, progress and such
		case "createActiveEffect":
			subtitle = `${i18n(MODULE_NAME+".ChatCard.AddEffect")} ${tokenData.name}`
			break;
		default:
			break;
	}
	...
	const templateData = {
		...
		effects: effects,
		settings: {
			disable: game.settings.get(MODULE_NAME,"showDisable"),
			showinfo: game.settings.get(MODULE_NAME,"showShowInfo"),
			delete: game.settings.get(MODULE_NAME,"showDelete")
		},
		subtitle: subtitle,
		scene: scene
	}
	...
	const content = await renderTemplate(`/modules/${MODULE_NAME}/templates/chatmessage.hbs`,templateData);
	...
	switch(game.settings.get(MODULE_NAME,"effectMessageMode")){
		case "gmwhisper":
	...
	if (tokenData.hidden){
		switch(game.settings.get(MODULE_NAME,"hiddenTokenOverride")){
			case "gmwhisper":
...
export const _onRenderChatMessage = async function(app, html, data) {
	...
	deleteEffectAnchor?.on("click", event => {
		async function anchorDelete(token, effectId, effectName){
			...
			if(! await token.actor.effects.get(effectId)?.delete()){
				content = i18n(MODULE_NAME+".ChatCard.NoEffect")
			} else {content = game.i18n.format(MODULE_NAME+".ChatCard.EffectDeleted", {name: effectName})};
			return content
		}
	...
	disableEffectAnchor?.on("click", event => {
		async function anchorDisable(token, effectId, effectName) {
			...
			if(! await token.actor.effects.get(effectId)?.update({disabled: true})){
				content = i18n(MODULE_NAME+".ChatCard.NoEffect")
			} else {content = game.i18n.format(MODULE_NAME+".ChatCard.EffectDisabled", {name: effectName})};
			return content;
		}
...
export const _anchorHandler = async function(event, anchorAction){
	...
	//Confirm in chat
	if (game.settings.get(MODULE_NAME,"confirmationMode") === "none") return; //no chat confirmation

	const currentUser:any = game.userId;
	const gmUsers = game.users.filter(user => user.isGM && user.active);
	let whisperUsers = [];
	
	switch(game.settings.get(MODULE_NAME,"confirmationMode")){
		case "gmwhisper":
			whisperUsers.push(gmUsers);
